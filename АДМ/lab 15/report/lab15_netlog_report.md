...
6. В файле `/etc/rsyslog.d/netlog-client.conf` настроено перенаправление всех сообщений системного журнала на сервер `server.kolade.net` по TCP-порту 514 с использованием надёжного соединения.
...
9. Под пользователем `kolade` на сервере запущено графическое приложение `gnome-system-monitor` для контроля состояния системы и активных процессов.
